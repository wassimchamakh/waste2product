...
<div class="event-card card-hover bg-white rounded-lg shadow-sm border border-gray-200 event-type-{{ $event->type }} overflow-hidden">
    <div class="relative">
        ...
        <div class="absolute top-2 sm:top-3 left-2 sm:left-3">
            <span class="badge-{{ $event->type }} text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
        ...
        <div class="absolute top-2 sm:top-3 right-2 sm:right-3">
            @if($isFull)
                <span class="bg-accent text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">Complet</span>
            @else
                <span class="bg-success text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">Disponible</span>
            @endif
        </div>
        
        <!-- Days until -->
        <div class="absolute bottom-2 sm:bottom-3 right-2 sm:right-3">
            <span class="bg-white/20 backdrop-blur-sm text-white px-2 py-1 rounded text-xs sm:text-sm">
    ...
    <div class="p-4 sm:p-6">
        ...
        <div class="space-y-2 mb-3 sm:mb-4">
            <div class="flex items-center text-gray-600">
                <i class="fas fa-calendar-alt mr-2 text-primary text-sm"></i>
        ...
        <div class="mb-3 sm:mb-4">
            <div class="flex justify-between items-center mb-2">
                ...
                <span class="text-xs sm:text-sm font-medium">{{ round($progressPercent) }}%</span>
            </div>
            <div class="w-full bg-gray-200 rounded-full h-1.5 sm:h-2">
                <div class="progress-bar {{ $progressColor }} h-1.5 sm:h-2 rounded-full" style="width: {{ $progressPercent }}%"></div>
            </div>
        </div>
        ...
        <div class="mb-3 sm:mb-4">
            @if($event->price == 0)
                <span class="bg-success text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
                    <i class="fas fa-tag mr-1"></i>Gratuit
                </span>
            @else
                <span class="bg-secondary text-white px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium">
        ...
        <!-- Organizer -->
        <div class="mb-3 sm:mb-4 text-xs sm:text-sm text-gray-600 truncate">
