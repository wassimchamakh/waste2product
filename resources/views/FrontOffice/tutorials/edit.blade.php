    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center">
            <h1 class="text-3xl md:text-4xl font-bold mb-4">
                <i class="fas fa-edit mr-3"></i>Modifier le Tutoriel
            </h1>
...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    <div class="bg-white rounded-2xl shadow-xl p-8">
        ...
        <form action="{{ route('tutorials.update', $tutorial->slug) }}" method="POST" enctype="multipart/form-data" id="tutorial-form">
            @csrf
            @method('PUT')
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="Recycling" {{ old('category', $tutorial->category) == 'Recycling' ? 'selected' : '' }}>♻️ Recyclage</option>
                                ...
                                <option value="Waste Reduction" {{ old('category', $tutorial->category) == 'Waste Reduction' ? 'selected' : '' }}>🗑️ Réduction des déchets</option>
                                <option value="Gardening" {{ old('category', $tutorial->category) == 'Gardening' ? 'selected' : '' }}>🌿 Jardinage</option>
                                <option value="DIY" {{ old('category', $tutorial->category) == 'DIY' ? 'selected' : '' }}>🔨 Bricolage</option>
                        ...
                        <div>
                            ...
                            >
                                ...
                                <option value="Beginner" {{ old('difficulty_level', $tutorial->difficulty_level) == 'Beginner' ? 'selected' : '' }}>
                                    🟢 Débutant
                                </option>
                                <option value="Intermediate" {{ old('difficulty_level', $tutorial->difficulty_level) == 'Intermediate' ? 'selected' : '' }}>
                                    🟡 Intermédiaire
                                </option>
                                <option value="Advanced" {{ old('difficulty_level', $tutorial->difficulty_level) == 'Advanced' ? 'selected' : '' }}>
                                    🟠 Avancé
                                </option>
                                <option value="Expert" {{ old('difficulty_level', $tutorial->difficulty_level) == 'Expert' ? 'selected' : '' }}>
                                    🔴 Expert
                                </option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <div class="w-full px-4 py-3 rounded-lg border border-gray-300 bg-gray-50 text-gray-700 text-base flex items-center gap-2">
                                ...
                                <span id="estimated_duration_display">{{ $tutorial->estimated_duration }} minutes</span>
                            </div>
                            <p class="text-sm text-gray-600 mt-1">
                                ⚡ Calculé automatiquement à partir des étapes
                            </p>
                            <!-- Hidden field for submission -->
                            <input type="hidden" id="estimated_duration" name="estimated_duration" value="{{ old('estimated_duration', $tutorial->estimated_duration) }}">
                        </div>
                            ...
                            <div class="flex gap-4">
                                <label class="flex items-center">
                                    <input 
                                        ...
                                        name="status" 
                                        value="Draft"
                                        {{ old('status', $tutorial->status) == 'Draft' ? 'checked' : '' }}
                                        class="mr-2"
                                        required
                                    >
                                    📝 Brouillon
                                ...
                                <label class="flex items-center">
                                    <input 
                                        ...
                                        name="status" 
                                        value="Published"
                                        {{ old('status', $tutorial->status) == 'Published' ? 'checked' : '' }}
                                        class="mr-2"
                                    >
                                    ✅ Publié
                                </label>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="mb-4">
                                ...
                                <img src="{{ $tutorial->thumbnail_url }}" alt="Image actuelle" class="max-h-48 rounded-lg shadow-lg">
                            </div>
                            @endif
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-600 focus:border-transparent transition-colors text-base"
                            >{{ old('prerequisites', $tutorial->prerequisites) }}</textarea>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea 
                                ...
                                class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-600 focus:border-transparent transition-colors text-base"
                            >{{ old('learning_outcomes', $tutorial->learning_outcomes) }}</textarea>
                        </div>
            ...
            <div class="mb-12">
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-2xl font-bold text-gray-900 flex items-center gap-3">
                        ...
                        Étapes du tutoriel
                        <span class="text-sm text-gray-500 font-normal">(Optionnel)</span>
                    </h2>
                    <button 
                        type="button" 
                        onclick="addStep()" 
                ...
                <div id="steps-container">
                    @php
                        $existingSteps = $tutorial->steps ?? collect();
                        $stepCount = $existingSteps->count();
                    @endphp
                    
                    @forelse($existingSteps as $index => $step)
                    <!-- Existing Step {{ $index + 1 }} -->
                    <div class="step-item bg-gray-50 rounded-xl p-6 mb-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button 
                                ...
                                class="text-red-500 hover:text-red-700 remove-step {{ $stepCount <= 1 ? 'hidden' : '' }}"
                        ...
                        <div class="grid grid-cols-1 gap-6">
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-600 focus:border-transparent text-base"
                                >{{ old('steps.' . $index . '.content', $step->content) }}</textarea>
                            </div>
                            ...
                            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input 
                                        ...
                                        value="{{ old('steps.' . $index . '.video_url', $step->video_url) }}"
                                        placeholder="https://youtube.com/watch?v=..."
                                        class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-600 focus:border-transparent text-base"
                                    >
                                </div>
                            ...
                            <div>
                                ...
                                <div class="mb-4">
                                    <p class="text-sm text-gray-600 mb-2">Image actuelle:</p>
                                    <img src="{{ $step->image_full_url }}" alt="Image de l'étape" class="max-h-48 rounded-lg shadow-lg">
                                </div>
                                @endif
                                    ...
                                    <div class="space-y-2 text-center">
                                        ...
                                        <div class="flex text-sm text-gray-600">
                                            <label class="relative cursor-pointer bg-white rounded-md font-medium text-green-600 hover:text-green-700 focus-within:outline-none focus-within:ring-2 focus-within:ring-offset-2 focus-within:ring-green-600 px-3 py-1">
                                                <span>{{ $step->image_url ? 'Changer l\'image' : 'Télécharger une image' }}</span>
                                                <input 
                                                    type="file" 
                            ...
                            <div>
                                ...
                                <textarea 
                                    ...
                                    rows="2"
                                    placeholder="Conseils utiles ou avertissements pour cette étape..."
                                    class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-green-600 focus:border-transparent text-base"
                                >{{ old('steps.' . $index . '.tips', $step->tips) }}</textarea>
                            </div>
                        </div>
                    </div>
                    @empty
                    <!-- Initial empty step if no existing steps -->
                    <div class="step-item bg-gray-50 rounded-xl p-6 mb-6">
                        <div class="flex items-center justify-between mb-4">
                            <h3 class="text-lg font-semibold text-gray-900 flex items-center gap-2">
...
<script>
let stepCount = {{ $existingSteps->count() > 0 ? $existingSteps->count() : 1 }};
...
document.addEventListener('DOMContentLoaded', function() {
    console.log('Formulaire de modification de tutoriel initialisé');
    updateRemoveButtons();
    attachDurationListeners();
    calculateTotalDuration();
});
