...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="mb-8">
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-gray-600 mt-2">Gérez toutes vos notifications</p>
            </div>
            <div class="flex items-center space-x-3">
                ...
                @endif
                @if($stats['read'] > 0)
                    <form action="{{ route('admin.notifications.delete-all-read') }}" method="POST" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer toutes les notifications lues ?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="px-4 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition-colors flex items-center space-x-2">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 mb-1">Non lues</p>
                    <h3 class="text-3xl font-bold text-blue-600">{{ $stats['unread'] }}</h3>
                </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-6 border border-gray-100">
            <div class="flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-600 mb-1">Lues</p>
                    <h3 class="text-3xl font-bold text-green-600">{{ $stats['read'] }}</h3>
                </div>
    ...
    <div class="bg-white rounded-xl shadow-sm p-4 mb-6 border border-gray-100">
        <div class="flex items-center space-x-4">
            ...
            <a href="{{ route('admin.notifications.index') }}" 
               class="px-4 py-2 rounded-lg text-sm {{ !request('filter') ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                Toutes
            </a>
            <a href="{{ route('admin.notifications.index', ['filter' => 'unread']) }}" 
               class="px-4 py-2 rounded-lg text-sm {{ request('filter') === 'unread' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                Non lues
            </a>
            <a href="{{ route('admin.notifications.index', ['filter' => 'read']) }}" 
               class="px-4 py-2 rounded-lg text-sm {{ request('filter') === 'read' ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                Lues
            </a>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        @forelse($notifications as $notification)
            <div class="border-b border-gray-100 last:border-b-0 {{ $notification->is_read ? 'bg-white' : 'bg-blue-50' }}">
