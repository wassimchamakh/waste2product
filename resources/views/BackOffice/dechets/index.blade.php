...
<div class="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 py-10">
    <div class="container mx-auto px-4 max-w-6xl">
        ...
        <div class="flex flex-col md:flex-row justify-between md:items-center mb-6 gap-4">
            <h1 class="text-3xl font-bold text-gray-800">Liste des Déchets</h1>
            <a href="{{ route('admin.dechets.create') }}" 
        ...
        <div class="bg-white rounded-xl shadow-lg p-6 mb-6">
            <form method="GET" action="{{ route('admin.dechets.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div class="bg-white rounded-xl shadow-lg overflow-hidden">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-200">
                    @forelse ($dechets as $dechet)
                        <tr class="hover:bg-gray-50 transition">
                            <td class="px-6 py-4 text-gray-800 font-medium">{{ $dechet->title }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ Str::limit($dechet->description, 60) }}</td>
                            <td class="px-6 py-4 text-gray-800">{{ $dechet->quantity }}</td>
                            <td class="px-6 py-4">
                                <span class="px-3 py-1 rounded-full text-xs font-medium 
                                    {{ $dechet->getStatusLabel() === 'Actif' 
                            ...
                            <td class="px-6 py-4 text-right space-x-2">
                                <a href="{{ route('admin.dechets.edit', $dechet->id) }}" 
                                   class="inline-block bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded-lg text-sm">
                                    Modifier
                                </a>
                                <form action="{{ route('admin.dechets.destroy', $dechet->id) }}" 
        ...
        <!-- Pagination -->
        <div class="mt-6">
            {{ $dechets->links('pagination::tailwind') }}
        </div>
    </div>
</div>
@endsection
